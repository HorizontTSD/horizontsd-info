...
function DesktopButton({ active = false, index, handleItemClick, content }: DesktopButtonProps) {
  ...
  const activeBackground = [
    ...
    `rgb(from var(--mui-palette-info-main) r g b / 0.9)`,
  ];
  const hoverBackground = [
    "rgb(from var(--mui-palette-info-main) r g b / 0.2)",
    "rgb(from var(--mui-palette-info-dark) r g b / 0.2)",
  ];
  ...
  return (
    <Box
      sx={{
        ...
        border: `none`,
        background: active ? activeBackground[~~isDark] : bgPalette[~~!isDark],
        cursor: "pointer",
        color: active ? `var(--mui-palette-primary-light)` : `var(--mui-palette-text-primary)`,
        display: "flex",
        flexDirection: "column",
        marginLeft: index != 0 ? "1rem" : 0,
        "&:hover": {
          background: active ? activeBackground[~~!isDark] : hoverBackground[~~isDark],
          color: active ? activeColor[~~isDark] : activeColor[~~!isDark],
        },
      }}
    ...
    >
      ...
      >
        <Stack direction={"column"}>
          ...
          <Typography variant={isSm ? "caption" : "body2"}>{content.description}</Typography>
        </Stack>
      </Box>
    </Box>
  );
      ...
      >
        ...
        >
          ...
          >
            ...
            <Stack>
              <NextLink href={selected.link || "/features"}>
                <Button
                  color="secondary"
                  variant="contained"
                  sx={{ background: "var(--mui-palette-info-main)", color: "primary.light" }}
                >
                  {dictionary.Button}
                </Button>
              </NextLink>
